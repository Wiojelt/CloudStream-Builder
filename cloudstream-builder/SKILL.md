...
- Repository install integrity invariant (Anti-"Hata" Lock): CloudStream compares downloaded `.cs3` files strictly against `fileHash` (`sha256-<hash>`), `hash`, and `fileSize`. Every build deployment MUST automatically compute exact file size and lowercase SHA-256 hash from the output `.cs3` and write them to `plugins.json`. Never push mismatched or stale hashes. Verify downloadable links with HTTP 200 prior to final push.
